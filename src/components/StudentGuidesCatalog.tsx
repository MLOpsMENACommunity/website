...
import { ArrowRight, Box, CalendarClock, Database, GitBranch, Github, GitPullRequest, Layers, Network, Search, Workflow, X } from 'lucide-react'
      ...
      {filteredGuides.length === 0 ? (
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {filteredGuides.map((guide, index) => {
            ...
            const Icon = isDocker ? Box : isDvc ? GitBranch : isAirflow ? Workflow : Github
            ...
            >
              ...
              <div className="relative flex h-full flex-col md:max-w-[68%]">
                <div className="flex items-start justify-between gap-4">
                  <span className={`${isDocker ? 'docker-card-icon' : isDvc ? 'dvc-card-icon' : isAirflow ? 'airflow-card-icon' : 'github-card-icon'} flex h-14 w-14 items-center justify-center rounded-2xl border border-cyan-400/25 bg-cyan-400/[0.08] text-cyan-400`}>
