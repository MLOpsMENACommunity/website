    ...
    <section className="hero-grid relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-content px-5 pb-16 pt-20 text-center sm:px-8 sm:pt-28">
        <div className="flex flex-col items-center gap-7">
          {whatsapp && (
            <span className="live-pill enter" style={step(0)}>
              <span className="live-dot" aria-hidden="true" />
              {copy.liveBadge(whatsapp.value)}
            </span>
          )}
          <span className="eyebrow enter" style={step(1)}>{copy.eyebrow}</span>
        </div>

        <h1 className="mx-auto mt-6 max-w-4xl text-4xl font-bold leading-[1.08] sm:text-5xl lg:text-6xl">
          <span className="hero-line-mask" style={step(2)}>
            <span>{copy.titleLine1}</span>
          </span>
          <span className="hero-line-mask" style={step(3)}>
            <span className="brand-text brand-text-animated text-shadow-glow">
              {copy.titleLine2}
            </span>
          </span>
        </h1>

        <p className="mx-auto mt-6 max-w-2xl text-lg leading-relaxed text-muted enter" style={step(4)}>
          {copy.lead}
        </p>

        <div className="mt-9 flex flex-wrap items-center justify-center gap-3 enter" style={step(5)}>
