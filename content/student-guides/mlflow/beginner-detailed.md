...
That is not carelessness. It is a missing system: **nothing was writing anything down.** Every fact about that run lived in your terminal scrollback and your memory.
...
**MLflow's central idea is a framework-agnostic contract.** A logged model is not "a pickle from someone's laptop" — it is a directory with a declared flavour, a signature describing inputs and outputs, and a dependency list. Anything that understands that contract can load it: a batch job, a REST server, a Spark UDF, a Docker image. That single property is what turns a training script into something deployable.
...
You need very little to follow along: Python, a script that trains something small, and a folder. A tiny dataset and a five-line model are genuinely the best place to start, because a broken run costs you nothing.
...
<ol class="guide-steps">
  ...
  <li><b>A model was logged</b>Not a bare pickle — a directory with an <code>MLmodel</code> file declaring its flavours, its signature, and its dependencies.</li>
</ol>
...
<div class="guide-try">
  ...
  <ol>
    ...
    <li>Log a metric with <code>step=0..9</code> in a loop and look at the Metrics tab — note that you get a chart, not a number.</li>
    <li>Set a tag, then set the same tag to a different value, and confirm it simply overwrites.</li>
...
The search syntax is worth learning because it is the same language the Python API uses:
...
And in code, which is how you build a report or a promotion gate:
...
<div class="guide-try">
  ...
  <ol>
    ...
    <li>Move the alias to version 2 and rerun the exact same loading code. Note that nothing in your code changed.</li>
...
<div class="callout tip">
  ...
  A model pickled with scikit-learn 1.5 and loaded under 1.2 may fail loudly or, worse, load and behave differently. That is precisely what the logged <code>requirements.txt</code> is for — so let the default <code>virtualenv</code> manager use it in anything that matters, and reserve <code>--env-manager local</code> for quick local checks.
</div>
...
<div class="callout tip">
  ...
  Hand-rolled metrics drift: one run computes AUC on probabilities, another on labels, a third excludes a class. <code>mlflow.evaluate</code> computes the same set the same way for every run, which is what makes a leaderboard trustworthy rather than merely tidy.
</div>
...
| Random seeds | Log them as parameters and actually set them |
...
<div class="guide-try">
  ...
  <ol>
    ...
    <li>Make an uncommitted edit, rerun, and confirm the commit tag is unchanged — the edit is invisible.</li>
...
<ol class="guide-steps">
  ...
  <li><b>Read the params, not your code</b>Especially with autolog, the recorded parameters are what actually ran.</li>
...
| Say what `log_model` actually writes? | A directory with `MLmodel`, flavours, signature, requirements |
