...
<div class="callout warn">
  ...
  Without a signature an endpoint takes columns in the wrong order or type and returns predictions that are quietly wrong. <code>infer_signature</code> is one line and turns a silent production bug into a 400 at the boundary.
</div>
...
| Seeds | Log as a param, and actually set them |
...
<ol class="guide-steps">
  ...
  <li><b>Explain the difference between a parameter, a metric, and a tag.</b>A parameter is immutable configuration stored as a string — logging it twice with a different value raises. A metric is a float with a step, so it is a series you can plot. A tag is a mutable string for organisation and search. The rule of thumb: a number you might plot is a metric, something you might reclassify later is a tag.</li>
  <li><b>What does <code>log_model</code> actually write?</b>Not a pickle — a directory containing an <code>MLmodel</code> manifest that declares the flavours it can be loaded as, the signature, and the Python version, plus the serialised model and three environment files. That manifest is what lets a REST server, a Spark UDF, or a Docker build load the model without knowing scikit-learn or PyTorch.</li>
