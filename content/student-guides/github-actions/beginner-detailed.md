...
You need remarkably little to follow along: a free GitHub account, the ability to make a commit, and a repository. Any repository, in any language — a brand-new empty one is genuinely the best place to experiment. Public repositories get unlimited free minutes, so a playground costs you nothing.

<div class="guide-try">
  <span class="ct">Try it</span>
  <ol>
    <li>Create a public repository called <code>actions-playground</code>. It can be completely empty.</li>
...
These two abbreviations appear in every conversation about Actions, so it is worth being precise about them. They are not tools — they are practices, and Actions is one way to implement them.
...
|---|---|
| "Did you run the tests?" | The tests ran, and here is the report |
...
<div class="guide-try">
  ...
  <ol>
    ...
    <li>Type the filename as <code>.github/workflows/notes.yml</code> — note how GitHub turns each slash into a folder as you type.</li>
...
And `${{ ... }}` is an **expression** — GitHub substitutes a real value there before the command runs. `runner.os` becomes `Linux`; `github.repository` becomes `your-name/your-repo`.
...
<div class="guide-timeline">
  ...
  <div class="guide-timeline-item"><span>5s</span><strong>Machine created</strong><small>A clean Ubuntu virtual machine booted. Your code was <b>not</b> on it — remember this.</small></div>
...
This is why `runs-on` is a property of a job rather than the workflow: each job is asking for its own computer. And it is why splitting a pipeline into jobs is a real decision with real consequences rather than just tidiness — two jobs run in parallel and finish sooner, but they cannot hand files to each other without extra machinery.

<div class="callout tip">
  <span class="ct">Start with one job and several steps</span>
  Reach for multiple jobs when you genuinely want parallelism or a hard ordering gate. Splitting too early creates the "my deploy job cannot find the build output" problem for no benefit.
</div>
...
The five cron fields are `minute hour day-of-month month day-of-week`, so `'0 3 * * *'` reads as "minute 0 of hour 3, any day, any month, any weekday". Two things surprise people: schedules are always **UTC**, never your local time, and they only run from your repository's default branch — testing one on a feature branch will drive you mildly mad.
...
A single step is one or the other — putting `run` and `uses` in the same step is an error, and it is a favourite interview trick question.
...
Those step options are worth knowing now because they solve real problems. `working-directory` runs the command somewhere other than the repository root. `shell` picks the interpreter — `bash`, `sh`, `pwsh`, `python`. `env` sets variables for that step alone. And `name` is what appears in the log, which is the difference between a readable run and a wall of truncated commands.
...
An action is referenced as `owner/repo@ref`, and the `ref` decides what code you actually execute:
...
The distinction is worth knowing even though you will not need self-hosted runners for a while: they are machines you own and register yourself, for when you need specific hardware such as a GPU, a paid licence, or access to a private network. Senior covers when that is justified and the risks it introduces.
...
These are substituted **before** the command runs, which is worth understanding: by the time your shell sees the line, `${{ github.ref }}` has already become literal text. That is why they work in any part of a workflow file, not just inside `run`.

<div class="guide-try">
  <span class="ct">Try it</span>
  <ol>
    <li>Add the printing step above to your workflow and run it from a branch.</li>
    <li>Note exactly what <code>github.ref</code> prints. Write it down.</li>
...
<div class="callout warn">
  ...
  A value written to <code>$GITHUB_ENV</code> is <b>not</b> readable in the step that wrote it — only from the next step onwards. And neither file crosses a <b>job</b> boundary; that needs job outputs, which come later on this page.
</div>
...
<div class="guide-try">
  ...
  <ol>
    ...
    <li>Now try echoing <code>$FOO</code> in the <em>same</em> step that wrote it — note that it is still empty.</li>
...
That `permissions:` block is worth noticing now even though Senior covers it properly: it controls what the automatic token is allowed to do, and declaring it narrows the token rather than widening it.
...
`hashFiles` produces a stable hash of the matched files. `restore-keys` is a fallback prefix tried when the exact key misses — a partial hit, still much better than nothing. Mid-level goes considerably deeper on key design, because getting it wrong is the difference between a cache that helps and one that silently does nothing.
...
`fail-fast` is worth understanding now. It defaults to `true`, which cancels every sibling job as soon as one fails. That is right when you want a fast red signal and wrong when you are diagnosing — a single failing cell cancelling the rest hides exactly the pattern you need to see.
...
The timeout is the one people skip and later regret. The default is six hours per job — a workflow waiting on something that will never arrive quietly consumes your entire monthly allowance before anyone notices.
...
The same shape for a JavaScript project — only the setup action and the commands change, which is the point:
...
<div class="guide-try">
  ...
  <ol>
    <li>Take this pipeline into a project you actually work on, adapting the commands to your language.</li>
  ...
  <em>a working pipeline on real code, and the confidence that comes from having built rather than read it. This exercise is worth more than the rest of the page combined.</em>
</div>
...
<div class="guide-try">
  ...
  <em>a badge that is genuinely live. It is also the fastest way to notice a broken pipeline in a repository you are not watching closely.</em>
</div>
