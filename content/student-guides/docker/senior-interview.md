Part three of three. A cumulative review of **the entire series**: foundations, image and runtime machinery, and the security and platform work a senior owns, organised by topic rather than by level. About fifty minutes. Fast review first, common questions at the end.
...
**Container versus VM:** shared kernel, milliseconds, tens of MB, isolated by kernel features. A VM boots a full guest OS in tens of seconds, costs gigabytes, and is isolated by a hypervisor. Container isolation is *weaker*, and you should say so unprompted.
...
**A container lives as long as its main process.** `EXPOSE` publishes nothing. `RUN` is build time, `CMD` is run time. Inside a container, bind `0.0.0.0` and log to stdout.
...
| Lever | Effect |
|---|---|
| Multi-stage | Drops the whole toolchain from the shipped image |
...
People forget `--start-period`. The check runs inside the container and its exit code is the result, so `curl -f` matters: plain curl exits 0 on a 500.
...
Limits: `-m` is a hard ceiling (exceeding it means SIGKILL and 137), `--cpus` throttles rather than fails, `--pids-limit` caps processes. Without limits the OOM killer may take **something else**. Older JVMs and Node builds read the host's memory rather than the cgroup, so a container can OOM at 512 MB while idle.

### Compose, registries, promotion

`depends_on` waits for **started**, not **ready**. Fix it with a health check plus `condition: service_healthy`, and retries in the app. `compose.override.yaml` applies automatically, so pass `-f` explicitly in CI. `docker compose config` prints the merged file and settles most disputes.
...
There is **no hypervisor**. Your container shares one kernel with the host and every other container.

<div class="callout warn">
  <span class="ct">Root in a container is root on the host kernel</span>
  Without user namespace remapping or a rootless daemon, uid 0 inside is uid 0 outside. An escape via a kernel bug, a careless bind mount, or an added capability lands as root on the host. For untrusted code you want gVisor, Kata, Firecracker, or a separate VM.
</div>

Docker's default capability set still includes `NET_RAW`, `SETUID`, `CHOWN`, and `MKNOD`, which most applications never use.
...
Automate the mechanical rows with hadolint, `trivy config`, and a `docker history` grep in CI. Reserve human review for judgement: is the capability set right, does the health check test the right thing, is that `--privileged` necessary.

## Common interview questions

<ol class="guide-steps">
  <li><b>What does container isolation consist of?</b>Three separate mechanisms: namespaces give separate views of PIDs, mounts, network, hostname, users, and IPC; cgroups impose limits on CPU, memory, PIDs, and I/O; capabilities, seccomp, and AppArmor or SELinux restrict which privileged operations the process may perform. There is no hypervisor, so the process runs on the host's kernel.</li>
  <li><b>Is a container a security boundary?</b>It is a real boundary, weaker than a VM, and not sufficient for untrusted code. Root inside is root on the host kernel unless you use userns-remap or a rootless daemon, so a kernel vulnerability or a careless mount is an escape path. For untrusted workloads, use gVisor, Kata, Firecracker, or a separate VM.</li>
  ...
  <li><b>A secret was found in a published image. What is your first move?</b><b>Rotate.</b> The image is already pulled and cached in CI runners, developer machines, and registry mirrors you do not control, so assume the credential is compromised regardless of how fast you delete the tag. Then revoke, clean up what you can reach, find the instruction that introduced it, and close the class with a secret mount plus a CI check.</li>
  ...
  <li><b>Why gate on fixable findings only?</b>Because failing the build on vulnerabilities with no available patch teaches the team to ignore scan output, which is worse than not scanning. Gate on what you can act on, track the rest, and treat image age as the metric that correlates with exposure.</li>
  ...
  <li><b>Is a shared build cache a trust boundary?</b>Yes. A cache is an input to your build, so if a fork's pull request can write to the cache your trusted branch reads, a poisoned entry gives the attacker code execution in a privileged build. Scope caches per branch or trust level, and never restore and execute cache contents without thought.</li>
  ...
  <li><b>Would you recommend Swarm?</b>For a small internal multi-host deployment it is simpler and defensible. As a general recommendation, no: the ecosystem and industry momentum are with Kubernetes, and choosing Swarm means owning that gap. Be ready for the follow-up either way.</li>
  <li><b>How would you run Docker for sixty engineers?</b>Publish hardened base images rather than documentation, because a <code>FROM</code> line gets adopted and a wiki page does not. Version them with a moving major tag and canary the exact patch first. Own registry policy: immutable tags, retention, an internal mirror, admission control. Measure image size, image age, fixable findings, and build duration, because without numbers "our images are fine" is an opinion.</li>
